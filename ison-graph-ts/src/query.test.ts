/**
 * Tests for ISONQL - TypeScript Implementation
 */

import { describe, it, expect, beforeEach } from 'vitest';
import { ISONGraph, Direction } from './index';
import {
  QueryEngine,
  QueryBuilder,
  EdgeQueryBuilder,
...
function createSimpleGraph(): ISONGraph {
...
function createSocialGraph(): ISONGraph {
  const graph = new ISONGraph();
...
describe('ISONQLParser', () => {
  ...
  describe('tokenization', () => {
    ...
    it('should tokenize query with string literals', () => {
      ...
      const conditions = result.conditions as Condition[];
    ...
    it('should tokenize multi-char operators', () => {
      ...
      const conditions = result.conditions as Condition[];
      expect(conditions[0].operator).toBe(Operator.GE);
    });
  });
  ...
  describe('NODES parsing', () => {
    ...
    it('should parse NODES with WHERE', () => {
      ...
      expect(conditions[0].operator).toBe(Operator.GT);
      expect(conditions[0].value).toBe(25);
    });
  ...
  describe('EDGES parsing', () => {
    ...
    it('should parse EDGES with WHERE', () => {
      ...
      const conditions = result.conditions as Condition[];
      expect(conditions[0].field).toBe('since');
    });
  });
...
describe('NODES Query', () => {
  ...
  it('should order results', () => {
    ...
    const ages = result.data.map((n: any) => n.properties.age);
...
describe('ORDER BY with missing fields', () => {
  function createPartialGraph(): ISONGraph {
    const graph = new ISONGraph();
  ...
  it('should sort nodes missing the field last (ASC)', () => {
    ...
    const names = result.data.map((n: any) => n.properties.name);
