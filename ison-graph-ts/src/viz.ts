 ...
 * (Python) exactly: the same graph, size, and seed produce bit-identical
...
export function layoutKey(ref: NodeRef): string {
 ...
 */
export function computeLayout(graph: ISONGraph, options: LayoutOptions = {}): Layout {
  ...
  const springMap = new Map<string, [string, string]>();
  for (const edge of graph.edges()) {
    ...
    const pair: [string, string] = ka <= kb ? [ka, kb] : [kb, ka];
...
function typeSlots(graph: ISONGraph): Map<string, number> {
...
function fillFor(slot: number, palette: string[], other: string): string {
  return slot < palette.length ? palette[slot] : other;
}

function radius(degree: number, dmin: number, dmax: number): number {
  if (dmax <= dmin) return 9;
  return 6 + (10 * (degree - dmin)) / (dmax - dmin);
}

function nodeLabel(node: Node, labelProperty?: string): string {
...
function* visibleEdges(graph: ISONGraph) {
 ...
 */
export function renderSvg(graph: ISONGraph, options: RenderOptions = {}): string {
  const width = options.width ?? 900;
  const height = options.height ?? 600;
...
function themeCss(
  palette: string[], other: string, surface: string,
  ink1: string, ink2: string, edge: string,
): string {
  const rules = [
 ...
 */
export function renderHtml(graph: ISONGraph, options: RenderOptions = {}): string {
  const width = options.width ?? 900;
  const height = options.height ?? 600;
  const layout = options.layout ?? computeLayout(graph, options);
